...
class TestUserInterface(unittest.TestCase):
    ...
    def test_valid_passowrd(self):
        print()
        print("-----------------------")
        print("Testing valid_passowrd(...)")
        print()
        username = "Tony2@859!"
        success = self.interface.valid_password(username, "hD53dj!85")
        print(f"Testing a valid password 'hD53dj!85' with username '{username}' -> Expected: True, Actual {success}")
        self.assertTrue(success)

        success = self.interface.valid_password(username, "hD53dj@85224fgsdaf")
